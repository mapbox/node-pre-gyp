...
function validate_config(package_json,callback) {
    ...
    if (!package_json.main) {
        throw new Error(msg+"package.json must declare 'main'");
    }
    if (!package_json.binary) {
        throw new Error(msg+"package.json must declare 'binary' property.\nSee https://github.com/springmeyer/node-pre-gyp#design for details\n");
    }
    var o = package_json.binary;
    if (!o.module_path) {
        throw new Error(msg+"package.json must declare 'binary.module_path'");
    }
    if (!o.module_name) {
        throw new Error(msg+"package.json must declare 'binary.module_name'");
    }
    if (!o.template) {
        throw new Error(msg+"package.json must declare 'binary.template'");
    }
    if (!o.remote_uri) {
        throw new Error(msg+"package.json must declare 'binary.remote_uri'");
    }
    // enforce `remote_uri` as https
    var protocol = url.parse(o.remote_uri).protocol;
    if (protocol !== 'https:') {
        throw new Error("'remote_uri' protocol ("+protocol+") is invalid - only 'https:' is accepted");
    }
};
...
module.exports.evaluate = function(package_json,options,callback) {
    ...
    var opts = {
        ...
        , module_path: package_json.binary.module_path
    ...
    opts.remote_uri = eval_template(package_json.binary.remote_uri,opts);
    opts.versioned = eval_template(package_json.binary.template,opts).replace('.tar.gz','');
    opts.versioned_path = path.join(package_json.binary.module_path,opts.versioned);
    opts.versioned_tarball = opts.versioned + '.tar.gz';
    return opts;
}
