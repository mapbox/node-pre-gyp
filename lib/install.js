...
try {
  http_get.impl = require('request'); // eslint-disable-line node/no-missing-require
...
function download(uri, opts, callback) {
  ...
  if (proxyUrl) {
    if (/^https?:\/\//i.test(proxyUrl)) {
      ...
      if (http_get.type === 'needle') {
        // needle does not handle proxy for https correctly, we have to set an agent ourself
...
function place_binary(from, to, opts, callback) {
  download(from, opts, (err, req) => {
    ...
    req.on('response', (res) => {
      ...
      if (http_get.type === 'needle' && Object.hasOwnProperty.call(res.headers, 'location') && res.headers.location !== '') {
        return;
      }
      if (hasResponse) {
        return;
