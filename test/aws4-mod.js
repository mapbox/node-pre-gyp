...
const credentialsCache = new LruCache(1000);
...
RequestSigner.prototype.matchHost = function(host) {
  const match = (host || '').match(/([^\.]+)\.(?:([^\.]*)\.)?amazonaws\.com(\.cn)?$/);
...
RequestSigner.prototype.signature = function() {
  ...
  let kDate, kRegion, kService, kCredentials = credentialsCache.get(cacheKey);
  if (!kCredentials) {
...
RequestSigner.prototype.canonicalHeaders = function() {
  ...
  return Object.keys(headers)
    .filter((key) => { return HEADERS_TO_IGNORE[key.toLowerCase()] == null; })
    .sort((a, b) => { return a.toLowerCase() < b.toLowerCase() ? -1 : 1; })
    .map((key) => { return key.toLowerCase() + ':' + trimAll(headers[key]); })
    .join('\n');
};

RequestSigner.prototype.signedHeaders = function() {
  return Object.keys(this.request.headers)
    .map((key) => { return key.toLowerCase(); })
    .filter((key) => { return HEADERS_TO_IGNORE[key] == null; })
    .sort()
    .join(';');
};
