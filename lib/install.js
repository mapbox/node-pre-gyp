...
function download(uri,opts,callback) {
    ...
    if (opts.remote_s3_sign===true) {
        getS3SignedUrl(opts, function(err, signedUrl) {
          if (err) {
              log.verbose('download could not get S3 Signed URL using default');
              doDownload(uri, opts);
          } else {
              doDownload(signedUrl, opts);
          }
        });
    } else {
        doDownload(uri, opts);
