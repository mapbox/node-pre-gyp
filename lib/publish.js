...
var fs = require('fs')
  ...
  , AWS = require("aws-sdk")
  , url = require('url')
  , config = require('rc')("node-pre-gyp",{acl:"public-read"});

function publish(gyp, argv, callback) {
    var package_json = JSON.parse(fs.readFileSync('./package.json'));
    versioning.evaluate(package_json, gyp.opts, function(err,opts) {
        if (err) return callback(err);
        var tarball = path.join('stage',opts.versioned);
        existsAsync(tarball,function(found) {
            if (!found) {
                return callback(new Error("Cannot publish because " + binary_module + " missing: run `node-pre-gyp rebuild` first"))
            }
            if(!config.accessKeyId  || !config.secretAccessKey) {
                ...
                s3.putObject(s3_opts, function(err, resp){
                    if(err) return callback(err);
                    console.log("It's Available here:", "http://"+s3_opts.Bucket+".s3.amazonaws.com/"+s3_opts.Key);
                    return callback();
                });
            }
        });
