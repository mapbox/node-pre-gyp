...
function place_binary(from,to,opts,callback) {
    opts.cache_dir = process.env.NODE_PRE_GYP_CACHE || path.join(os.homedir(), '.node-pre-gyp');
    opts.cache_path = path.join(opts.cache_dir, path.basename(from));
    if(!opts.ignore_cache) {
        retrieve_from_cache(opts,function(err,stream) {
            if(err) {
                unlink_and_ignore(opts.cache_path);
                return do_download();
            }
