...
function configure_cache_dir(opts) {
    try {
        opts.cache_dir = process.env.NODE_PRE_GYP_CACHE || process.env.npm_config_node_pre_gyp_cache_dir || path.join(os.homedir(), '.node-pre-gyp');
    } catch(e) {
...
function cache_local_package(gyp, callback) {
