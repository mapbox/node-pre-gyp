...
const mockS3Http = require('./util/s3_setup.js').get_mockS3Http();
mockS3Http('on');
const mocking = mockS3Http('get');
...
//
class Run extends EE {
  constructor({ package_json_path = './package.json', argv }) {
    super();
    ...
    cli_commands.forEach((command) => {
      this.commands[command] = (argvx, callback) => {
        log.verbose('command', command, argvx);
        return require('./' + command)(this, argvx, callback);
      };
    });
  ...
  parseArgv(argv) {
    ...
    this.package_json = JSON.parse(fs.readFileSync(package_json_path, 'utf8'));
...
Run.prototype.package = require('../package.json');

/**
 * nopt configuration definitions
 */

Run.prototype.configDefs = {
