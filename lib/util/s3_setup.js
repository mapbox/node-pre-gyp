...
const URI_REGEX = '^(.*)\.(s3(?:-.*)?)\.amazonaws\.com$'; // eslint-disable-line no-useless-escape

module.exports.detect = function(to, config) {
  const uri = url.parse(to);
  const hostname_matches = uri.hostname.match(URI_REGEX);
  ...
  if (!config.region) {
    const s3_domain = hostname_matches[2];
