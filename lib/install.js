...
function install(gyp, argv, callback) {
    ...
    if (should_do_source_build) {
        ...
        var from = url.resolve(base,opts.versioned_tarball);
        var binary_module = path.join(opts.versioned_path,opts.module_name + '.node');
            ...
            if (found) {
                test_binary.validate(opts,function(err) {
                    if (err) {
                        ...
                        place_binary(from,opts.versioned_path,opts,function(err) {
