...
async function place_binary(uri, targetDir, opts, callback) {
  log.http('GET', uri);
  ...
  //const fetch = (await import('node-fetch')).default(sanitized, { agent });
  (await import('node-fetch')).default(sanitized, { agent })
    .then((res) => {
      if (!res.ok) {
...
function install(gyp, argv, callback) {
  ...
  if (should_do_source_build) {
    ...
    existsAsync(binary_module, (found) => {
      ...
      makeDir(to).then(async () => {
        const fileName = from.startsWith('file://') && from.slice('file://'.length);
        if (fileName) {
          extract_from_local(fileName, to, after_place);
        } else {
          await place_binary(from, to, opts, after_place);
        }
      }).catch((err) => {
        after_place(err);
      });
