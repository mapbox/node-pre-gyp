...
if (previous_patch_version && previous_patch_version !== current_version) {
  ...
  test(app.name + ' builds with custom --target=' + previous_patch_version + ' that is greater than known version in ABI crosswalk ' + app.args, (t) => {
    if (found) {
      ...
      run('node-pre-gyp.js', 'rebuild', '--build-from-source --fallback-to-build --target=' + previous_patch_version, app, opts, (err, stdout) => {
        t.ifError(err);
        t.notEqual(stdout, '');
        // For some reason there is no stderr starting with node v14
        // t.notEqual(stderr, '');
        run('node-pre-gyp.js', 'clean', '--target=' + current_version, app, opts, (err2, stdout2) => {
          t.ifError(err2);
          t.notEqual(stdout2, '');
          // For some reason there is no stderr starting with node v14
          // t.notEqual(stderr2, '');
