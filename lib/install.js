    ...
    if (req) {
        req.on('response', function (res) {
            ...
            if (res.statusCode !== 200) {
                var err = new Error(res.statusCode + ' status code downloading tarball ' + uri);
                err.statusCode = res.statusCode;
                return callback(err);
            }
            cache.write_to_cache(req, opts);
            callback(null,req);
        });
    }
}

function extract(source,to,callback) {
    ...
    source.pipe(gunzip).pipe(extracter)
      .on('close', after_tarball).on('error', callback);

    var badDownload = false;
    var extractCount = 0;

    function after_tarball(err) {
    ...
    gunzip.on('error', after_tarball);
    extracter.on('entry', filter_func);
    extracter.on('end', after_tarball);
...
function place_binary(from,to,opts,callback) {
    if(!opts.ignore_cache) {
        cache.retrieve_from_cache(opts,function(err,stream) {
            if(err) {
                return do_download();
            }
            extract(stream,to,function(err) {
                if(err) {
                    cache.unlink_and_ignore(opts.cache_path);
                    return do_download();
                }
                log.verbose('cache', 'binding extracted from cache');
                callback();
            });
        });
    } else {
    ...
    function do_download() {
        download(from,opts,function(err,stream) {
            if(err) {
                return callback(err);
            }
            extract(stream,to,callback);
        });
    }
}
...
function install(gyp, argv, callback) {
    ...
    if (should_do_source_build) {
        ...
        var no_caching = process.env.npm_config_node_pre_gyp_cache === 'false';
        opts.ignore_cache = no_caching || gyp.opts['ignore-node-pre-gyp-cache'];
        opts.skip_cache = no_caching || gyp.opts['skip-node-pre-gyp-cache'];
        cache.configure_cache_path(opts);
