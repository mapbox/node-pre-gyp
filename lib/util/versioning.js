...
module.exports.evaluate = function(package_json, options, napi_build_version) {
  ...
  // IBM i identifies itself as 'aix' when process.platform is called, but
  // binaries built for IBM i won't work for AIX. Instead, if the platform
  // identified itself as 'aix' and it isn't a cross build, check if os.type()
  // returned 'OS400' (IBM i). If so, change the platform name to 'os400' to
  // ensure correct packaging
  if (opts.platform === 'aix' && options.target_platform !== 'aix') {
    if (os.type() === 'OS400') {
      opts.platform = 'os400';
      opts.target_platform = 'os400';
    }
  }
