...
var do_package = require('./package.js');
...
function do_build(gyp,argv,callback) {
    handle_gyp_opts(gyp,argv,function(err,result) {
        ...
        compile.run_gyp(final_args,result.opts,function(err) {
            ...
            if(!(process.env.npm_config_node_pre_gyp_cache === 'false' || gyp.opts['skip-node-pre-gyp-cache'])) {
                do_package(gyp, argv, function(err) {
                    if(err) return callback();
                    cache.cache_local_package(gyp, argv, function() {
                        callback();
                    });
                });
            } else {
