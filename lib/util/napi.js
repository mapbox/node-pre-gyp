...
module.exports.validate_package_json = function(package_json) { // return err
	...
	var remote_path_ok = binary.remote_path && binary.remote_path.includes('{napi_build_version}');
	var package_name_ok = binary.package_name && binary.package_name.includes('{napi_build_version}');
	var napi_build_versions = get_napi_build_versions(package_json);
	...
	if (napi_build_versions && !module.exports.get_best_napi_build_version(package_json)) {
		throw new Error(
			...
			'This module supports N-API version(s) ' + get_napi_build_versions(package_json) + '. ' +
...
function get_napi_build_versions (package_json) {
...
module.exports.get_napi_build_version_from_command_args = function(command_args) {
	for (var i = 0; i < command_args.length; i++) {
		...
		if (arg.indexOf(napi_build_version_tag) === 0) {
			return parseInt(arg.substr(napi_build_version_tag.length));
		}
	}
	return undefined;
};
