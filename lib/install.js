...
function place_binary(uri, targetDir, opts, callback) {
  ...
  if (proxyUrl) {
    //               1                 2         3          4
    // const re = /^(?:(https?:)\/\/)?(?:([^@]+)@)?([^:]+)(?::(\d{1,5}))?\/?$/;
    // const m = proxyUrl.match(re);
    //                             1              2          3
    // const m = proxyUrl.match(/^(?:(https?:)\/\/)?([^:]+)(?::(\d{1,4}))?\/?$/);
    if (proxyUrl) {
      const ProxyAgent = require(/^https:/.test(proxyUrl) ? 'https-proxy-agent' : 'http-proxy-agent');
      // const ProxyAgent = require(m[1] === 'https:' ? 'https-proxy-agent' : 'http-proxy-agent');
      // const protocol = m[1] === 'https:' ? 'https:' : 'http:';
      // const host = m[3];
      // const port = +(m[4] || (protocol === 'https:' ? 443 : 80));
