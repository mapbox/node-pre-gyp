...
function get_node_abi_for_process(runtime, versions) {
    if (!runtime) {
        throw new Error("get_node_abi_for_process requires valid runtime arg");
    }
    if (!versions) {
        throw new Error("get_node_abi_for_process requires valid process.versions object");
    }
...
module.exports.get_node_abi_for_process = get_node_abi_for_process;

function get_node_abi_for_target(runtime, target_version, target_v8_version, target_abi) {
    if (!runtime) {
        throw new Error("get_node_abi_for_process requires valid runtime arg");
    }
    if (!target_version || !target_v8_version || !target_abi) {
        throw new Error("get_node_abi_for_target requires valid target_version, target_v8_version, and target_abi args");
    }
    var sem_ver = semver.parse(target_version);
    if (sem_ver.major === 0 && sem_ver.minor % 2) {
        return runtime+'-'+target_version;
    } else {
        return target_abi ? runtime+'-v' + (+target_abi) :
            'v8-' + target_v8_version.split('.').slice(0,2).join('.');
    }
}
module.exports.get_node_abi_for_target = get_node_abi_for_target;
...
// normalize is used for removing double slashes
...
module.exports.evaluate = function(package_json,options) {
    ...
    opts.remote_path = package_json.binary.remote_path ? fix_slashes(eval_template(package_json.binary.remote_path,opts)) : default_remote_path;
